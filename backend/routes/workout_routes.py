...
@workout_bp.put("/workouts/<int:workout_id>")
def update_logged_workout(workout_id):
    # Validate user
    user_id = session.get("user_id")
    if not user_id:
        return jsonify({"error": "Unauthorized"}), 401
    ...
    if not name or not workout_date:
    ...
    try:
        # Verify user owns the workout and it's not a template
        cur.execute("""
        ...
        cur.execute(
            """
            UPDATE workouts
            SET name = %s, notes = %s, workout_date = %s
            WHERE workout_id = %s
            """,
            (name, notes, workout_date, workout_id),
        )

        # Delete old exercise entries for this workout
        cur.execute("""
...
def get_workout_exercises(workout_id):
    ...
    try:
        ...
        # Fetch exercises - NOW INCLUDING CATEGORY
        cur.execute("""
